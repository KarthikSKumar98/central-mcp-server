...
1. Call `central_get_sites` with view="summary" to get all sites with health scores and device/client/alert counts; follow next_cursor until null to cover all results.
...
5. If no prioritized sites exist, skip steps 3-4 and summarize directly from the `central_get_sites` view="summary" output.
...
1. Call `central_get_sites` with view="summary" to verify the site name and get its site_id and current health score.
...
5. Call `central_get_events` with mode="facets", site_id=<site_id>, time_range="last_1h", response_mode="compact" to identify dominant event types.
...
1. Call `central_get_clients` with mac_address="{mac_address}".
...
5. If connected_device_serial is present, call `central_get_devices` with serial_number=<connected_device_serial> and inspect the single item in the returned DeviceEnvelope.
...
6. If site_id is present, map connection_type to events context_type (Wireless -> WIRELESS_CLIENT, Wired -> WIRED_CLIENT).
   If client status is "Failed" or "Disconnected", call `central_get_events` with mode="facets", site_id=<site_id>, context_type=<mapped type>, context_identifier="{mac_address}", response_mode="compact", using start_time/end_time from step 3 when last_seen_at is available, or time_range="last_24h" fallback when it is not.
   ...
   If client status is "Connected", call `central_get_events` with mode="facets", site_id=<site_id>, context_type=<mapped type>, context_identifier="{mac_address}", time_range="last_24h", response_mode="compact" and summarize that 24-hour event-facets output.
...
7. Summarize with these sections:
   ...
   - Recent client events:
     ...
     - Connected clients: last 24-hour event counts from `central_get_events` with mode="facets".
...
1. Call `central_get_devices` with serial_number="{serial_number}" to confirm the device exists and get its site_id and device_type from the single DeviceEnvelope item. If items is empty, report the serial number could not be located and stop.
2. Treat `central_get_devices.device_type` and `central_get_events.context_type` as separate schemas. Map device_type `ap` or `ACCESS_POINT` -> events context_type `ACCESS_POINT`; `switch` or `SWITCH` -> `SWITCH`; `gateway` or `GATEWAY` -> `GATEWAY`.
3. Call `central_get_events` with mode="facets", site_id=<site_id>, context_type=<mapped type>, context_identifier="{serial_number}", time_range="{time_range}", response_mode="compact" to discover top event_id/category/source_type values.
...
1. Call `central_get_sites` with view="summary" to verify the site name and get its site_id.
2. Call `central_get_events` with mode="facets", site_id=<site_id>, time_range="{time_range}", response_mode="compact" to get ranked event_id/category/source_type values.
...
1. Call `central_get_sites` with view="summary" to verify the site name and get its site_id.
2. Call `central_get_clients` with site_id=<site_id> and status="Failed" to get all failed clients; follow next_cursor until null to cover all results.
3. If no failed clients are found, report the site is clean.
4. Collect the unique connected_device_serial values from the failed clients (up to 5 unique serials). Call `central_get_devices` with serial_number=<connected_device_serial> once per unique serial and inspect the single DeviceEnvelope item — do not call it multiple times for the same serial.
...
1. Call `central_get_sites` with view="summary" to verify the site name and get its site_id.
2. Call `central_get_clients` with site_id=<site_id> to get all clients at the site; follow next_cursor until null to cover all results.
...
1. Call `central_get_sites` with view="summary" to verify the site name and get its site_id.
2. Map the requested type for `central_get_devices.device_type`: "Access Point" -> ap, "Switch" -> switch, "Gateway" -> gateway.
3. Call `central_get_devices` with site_id=<site_id> and the mapped lowercase device_type to list all matching devices; follow next_cursor until null to cover all results.
...
5. Separately map the requested type for `central_get_events.context_type`: "Access Point" -> ACCESS_POINT, "Switch" -> SWITCH, "Gateway" -> GATEWAY.
6. For up to 3 devices that have associated alerts, call `central_get_events` with mode="facets", site_id=<site_id>, context_type=<mapped uppercase events context type>, context_identifier=<serial_number>, time_range="last_1h", response_mode="compact".
7. For each device from step 6 where total > 0, call `central_get_events` with the same context and the top event_id and category filters from the count output.
8. Summarize: total device count, provisioned vs unprovisioned, active alert breakdown by severity, and devices with high event activity from tool output only. Do not provide remediation steps; direct remediation to Central.
        """.strip()
...
1. Call `central_get_sites` with view="summary" and resolve site_id for "{site_name}".
2. Call `central_get_events` with mode="facets", site_id=<site_id>, time_range="{time_range}", response_mode="compact".
...
1. Call `central_get_sites` with view="summary" to get all sites with their site_ids and alert counts.
...
2. Call `central_get_wlans` with wlan_name="{wlan_name}", include=["throughput"], and time_range="{time_range}" to get throughput trends. Interpret tx as transmitted throughput and rx as received throughput, both in bits per second.
...
1. Call `central_get_sites` with view="summary" to verify all site names and get their site_ids and health scores.
