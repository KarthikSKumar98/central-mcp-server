...
- `central_get_client_analytics` — one tool for the client-analytics family, selected by `metric`:
  - `usage`: byte usage samples (`view="usage"`), top clients by usage (`view="top"`, `top_n`), or client-count trends grouped by type, role, VLAN, WLAN, radio, security, or protocol (`view="trend"`).
  - `mobility`: one wireless client's roam trail by `mac_address`, paginated with `limit`/`cursor`.
  - `onboarding`: per-stage (assoc, auth, dhcp, dns) experience. `view="summary"` (default) returns attempts, failures, on-time successes, delays, and Central's stage success rate (`success_percent`, delayed attempts count as successful) per stage plus the top failure and delay reasons, with the overall 0–100 experience score on the envelope's `overall_score`; `response_format="detailed"` adds the top five clients, access devices, WLANs, bands, and servers behind failed and delayed attempts. `view="reasons"` and `view="count"` return top reasons and grouped counts by `status` and `field`. Filters: `stage`, `site_id`, `version`, `view_type`.
  - Numeric values Central reports as strings are returned as numbers; the `-1` no-data sentinel becomes `null`. Default `time_range` is `last_24h`.

### Changed

- The tool surface grows from 12 to 13 tools. Envelope, cursor, error, and `response_format` contracts are unchanged.
