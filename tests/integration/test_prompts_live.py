...
async def live_seed_data(sites_tools, devices_tools, clients_tools, live_ctx):
    ...
    mapping = {item.name: item.model_dump() for item in summary.items}
    if not mapping:
    ...
    devices = await devices_tools["central_get_devices"](
        live_ctx, site_id=first_site_id
    )
    first_device = devices.items[0] if devices.items else None
...
async def test_prompt_network_health_overview_live(
    ...
    assert "central_get_sites" in prompt_text
    assert 'view="summary"' in prompt_text

    t0 = time.perf_counter()
    try:
        summary = await sites_tools["central_get_sites"](live_ctx, view="summary")
        site_names = [item.name for item in summary.items[:3]]
...
async def test_prompt_troubleshoot_site_live(
    ...
    try:
        ...
        await devices_tools["central_get_devices"](
            live_ctx, site_id=live_seed_data["site_id"]
        )
    except Exception as exc:
        if _is_rate_limited_error(exc):
...
async def test_prompt_client_connectivity_check_live(
    ...
    assert "mac_address" in prompt_text
    assert "central_get_devices" in prompt_text

    t0 = time.perf_counter()
    found = await clients_tools["central_get_clients"](
        live_ctx,
        mac_address=client.mac,
    )
    if not found.items:
        pytest.skip("Unable to fetch client details")
    found_client = found.items[0]
...
async def test_prompt_investigate_device_events_live(
    ...
    try:
        await devices_tools["central_get_devices"](
            live_ctx, serial_number=device.serial_number
        )
        facets = await events_tools["central_get_events"](
            live_ctx,
        ...
        if _is_rate_limited_error(exc):
            pytest.skip(f"Rate limited by Central API: {exc}")
        raise
    if _is_rate_limited_result(facets):
        pytest.skip(f"Rate limited by Central API: {facets}")
    facet = facets.items[0]
    if facet.total > 0:
...
async def test_prompt_failed_clients_investigation_live(
    ...
    prompt_text = prompt_registry["failed_clients_investigation"](
        live_seed_data["site_name"]
    )
    ...
    if isinstance(failed, list):
        for client in failed[:5]:
            if client.connected_device_serial:
                await devices_tools["central_get_devices"](
...
async def test_prompt_device_type_health_live(
    ...
    assert "central_get_devices.device_type" in prompt_text
    assert "central_get_events.context_type" in prompt_text
    ...
    assert f'"{device_type}" -> {normalized}' in prompt_text
    event_context = {
        "Access Point": "ACCESS_POINT",
        "Switch": "SWITCH",
        "Gateway": "GATEWAY",
    }[device_type]
    assert f'"{device_type}" -> {event_context}' in prompt_text
...
async def test_prompt_top_event_drivers_live(
    ...
    if _is_rate_limited_result(facets):
        pytest.skip(f"Rate limited by Central API: {facets}")
    facet = facets.items[0]
    event_ids = ",".join(item.event_id for item in facet.event_names[:3]) or None
    categories = ",".join(facet.categories[:2]) or None
    if facet.total > 0 and (event_ids or categories):
        await events_tools["central_get_events"](
            live_ctx,
            site_id=live_seed_data["site_id"],
            time_range="last_24h",
...
async def test_prompt_critical_alerts_review_live(
    ...
    summary = await sites_tools["central_get_sites"](live_ctx, view="summary")
    critical_sites = [
        item.site_id for item in summary.items if item.critical_alerts > 0
    ][:3]
...
async def test_prompt_compare_site_health_live(
    ...
    try:
        summary = await sites_tools["central_get_sites"](live_ctx, view="summary")
        mapping = {item.name: item.site_id for item in summary.items}
        await sites_tools["central_get_sites"](live_ctx, site_names=site_names)
        for name in site_names:
            site_id = mapping[name]
            alerts = await alerts_tools["central_get_alerts"](
                live_ctx, site_id=site_id, status="Active"
            )
            if _is_rate_limited_result(alerts):
