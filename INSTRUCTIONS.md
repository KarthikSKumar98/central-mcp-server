...
- Use `central_get_client_analytics` for client experience questions. `metric="onboarding"` (default `view="summary"`) returns one item per stage (`assoc`, `auth`, `dhcp`, `dns`) with attempts, failures, success, delays, `success_percent`, and top failure/delay reasons; the envelope's `overall_score` (0–100) is the product of the per-stage success rates, so a low score with high per-stage rates is expected. Use `view="reasons"` or `view="count"` with `status` (and `field` for count) to drill into failures; `metric="usage"` for byte usage, top clients, or client-count trends; `metric="mobility"` with `mac_address` for a roam trail. Scope with `site_id` from `central_get_sites`; the default window is the last 24 hours.
